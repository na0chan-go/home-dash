...
- 新規実装・不具合修正時は必ずテストコードを追加または更新する
...
- レビュー指摘を修正したら、該当コメントへ「修正内容 + `@codex review`」で返信する
- 実装完了後も、レビュー前提で必ず再読し、境界条件・バリデーション・依存方向を深掘りして確認する
- 深掘り確認では最低限、以下を実施する
  - 正常系だけでなく、異常系（未指定・不正値・未知キー・旧フォーマット・上限/下限）をテストで確認する
  - API境界（400/404/500）とエラーメッセージを確認する
  - usecaseがportsのみに依存しているか、infra import漏れがないか確認する
  - 変更機能の実行確認（ローカル起動・主要API呼び出し）を行い、結果をPR本文またはコメントに残す
